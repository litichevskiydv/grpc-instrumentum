 ...
 * @returns {string}
 */
const preparePluginOptions = (pluginName, outputDirectory) =>
  `--${prepareScriptName(pluginName)}_out=${outputDirectory}`;
...
const generateJs = (argv, env, filesList) => {
  ...
  execFileSync(
    scriptName,
    [jsOutOption, `--grpc_out=grpc_js:${argv.out}`, ...prepareIncludes(argv.include), argv.protoFile],
    { env }
  );
};
